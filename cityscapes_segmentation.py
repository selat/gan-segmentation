...
def mask_name_to_image(mask_name):
    return mask_name[:-16] + 'leftImg8bit.png'

all_rows = np.array([[  0,   0,   0, 255],
       [  0,   0, 142, 255],
       [ 70,  70,  70, 255],
       [ 70, 130, 180, 255],
       [102, 102, 156, 255],
       ...
       [220,  20,  60, 255],
       [220, 220,   0, 255],
       [244,  35, 232, 255]], dtype=np.uint8)
    
def load_data(rows, cols, crops_num=30, start_index=1):
    imgs_train, imgs_mask_train = [], []
    
    mask_pref_path = '/mnt/data1T2/datasets2/Cityscapes_Dataset/data/gtFine_trainvaltest.unzip/gtFine/train/hamburg'
    img_pref_path = '/mnt/data1T2/datasets2/Cityscapes_Dataset/data/gtFine_trainvaltest.unzip/gtFine/cityscapes-gtfine-person/'
    for f in os.listdir(mask_pref_path):
        if f[-9:] != 'color.png':
            continue
        img = skio.imread(os.path.join(img_pref_path, mask_name_to_image(f)))
        mask = skio.imread(os.path.join(mask_pref_path, f))
        mask = np.all(mask == all_rows[7], axis=-1).astype(np.float32)
        for j in range(crops_num):
            ...
            imgs_mask_train.append(mask_crop)
...
def show_sample_segmentation(rows, model, path):
    ...
    pref_path = '/mnt/data1T2/datasets2/Cityscapes_Dataset/data/gtFine_trainvaltest.unzip/gtFine/cityscapes-gtfine-person/'
    all_files = filter(lambda x : x[:7] == 'hamburg' and x[-5:] == 't.png',
                       os.listdir(pref_path))
    for i in range(rows):
        img_id = random.randint(0, len(all_files))
        super_input = skio.imread(os.path.join(pref_path, all_files[img_id]))
        tmp = super_input.astype(np.float32)
        tmp /= 127.5
        tmp -= 1.0
        res = tmp_model.predict(np.array([tmp]))
        plots[i][0].axis('off')
        plots[i][0].imshow(super_input)
