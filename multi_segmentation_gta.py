...
def load_data(rows, cols, crops_num=30, start_index=1):
    ...
    for i in range(start_index, 1000 + start_index, 10):
        ...
        masks = np.zeros((mask.shape[0], mask.shape[1], len(all_rows_gta)), dtype=np.float32)
        for i, r in enumerate(all_rows_gta):
            masks[:,:,i] = np.all(mask == r, axis=-1)
        for j in range(crops_num):
            ...
            mask_crop = masks[startrow:startrow + rows, startcol:startcol + cols]
...
def get_unet_small(rows, cols):
    ...
    conv10 = Conv2D(14, (1, 1), activation='softmax')(conv9)
